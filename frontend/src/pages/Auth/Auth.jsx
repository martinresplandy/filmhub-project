  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.token) {
        ...
        if (data.username) {
          const mesageError = Array.isArray(data.username)
            ? data.username[0]
            : data.username;
          setError(
            mesageError === "This field may not be blank."
              ? "Username field may not be blank."
              : mesageError
          );
        } else if (data.email) {
          const mesageError = Array.isArray(data.email)
            ? data.email[0]
            : data.email;
          setError(
            mesageError === "This field may not be blank."
          ...
          const mesageError = Array.isArray(data.password)
            ? data.password[0]
            : data.password;
          setError(
            mesageError === "This field may not be blank."
